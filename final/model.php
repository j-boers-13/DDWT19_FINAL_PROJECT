...
function get_tenant_invites($pdo){
    ...
    $stmt = $pdo->prepare('SELECT viewing_invites.*, rooms.square_meters, address.street_address, address.city, address.zipcode FROM viewing_invites INNER JOIN rooms ON viewing_invites.room_id = rooms.id JOIN address ON rooms.address_id = address.id WHERE tenant_id = ? AND confirmed = "No"');
    $stmt->execute([$_SESSION['user_id']]);
    $optins = $stmt->fetchAll();
    $optins_exp = Array();
